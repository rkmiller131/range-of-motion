...
export const BubbleActions = {
  spawnBubble: defineAction(
    WorldNetworkAction.spawnObject.extend({
      type: 'uvx.bubble-game.BUBBLE_SPAWN'
    })
  ),
  popBubble: defineAction({
    type: 'uvx.bubble-game.BUBBLE_POP',
    entityUUID: matchesEntityUUID,
    playerPopped: boolean,
    position: matchesVector3.optional(),
    velocity: matchesVector3.optional()
  })
}

export type ScoreType = Array<{
  time: number
    ...
    onPopBubble: BubbleActions.popBubble.receive((action) => {
      ...
      getMutableState(BubbleScoreState)[action.entityUUID].merge([
        // when popBubble gets dispatched, these actions are passed. See BubblePhysicsSystem for dispatch
        {
          ...
          popped: action.playerPopped ? (prev) => prev + 1: (prev) => prev + 0,
          missed: !action.playerPopped ? (prev) => prev + 1: (prev) => prev + 0
        }
      ])
    })
  }
